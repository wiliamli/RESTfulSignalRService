    ...
    public sealed class RedisCache
    {
        ...
        public T Get<T>(string key)
        {
            ...
            lock (objLock) // 线程安全，单线程操作
            {
                ...
                {
                    t = Newtonsoft.Json.JsonConvert.DeserializeObject<T>(Database.StringGet(key));
                }
            }
            return t;
        }
        ...
        public bool Set<T>(string key, T value, int expireTime)
        {
            ...
            lock (objLock) // 线程安全，单线程操作
            {
                result = Database.StringSet(key, Newtonsoft.Json.JsonConvert.SerializeObject(value), new TimeSpan(0, 0, expireTime));
            }
            return result;
        }
